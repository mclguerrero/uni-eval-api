...
class PrismaSchemaParser {
  ...
  /**
   * Parsea una línea de campo individual
   */
  parseField(line) {
  ...
  fieldsToJsonSchema(fields, options = {}) {
    ...
    for (const field of fields) {
      // Ignorar campos de relación (inician con minúscula y son objetos Prisma)
  ...
  fieldToProperty(field) {
    ...
    if (!baseType) {
      // Tipo no reconocido (probablemente una relación)
      return null;
    }
    ...
    // Agregar descripción desde @db o comentarios si existen
  ...
  /**
   * Determina si un campo es una relación
   */
  isRelationField(field) {
    // Si el tipo no está en nuestro mapeo, probablemente es una relación
  ...
  shouldSkipField(field, options = {}) {
    ...
    // Siempre omitir campos de relación
    if (this.isRelationField(field)) {
      return true;
    }
  ...
  /**
   * Extrae descripción del campo (si existe en comentarios)
   */
  extractDescription(field) {
    // Buscar comentarios en la misma línea o línea anterior
...
function generateJsonSchemas(schemaPath, outputDir, options = {}) {
  console.log('🔄 Iniciando generación de JSON Schemas...');
  
  const parser = new PrismaSchemaParser(schemaPath);
  
  // Leer y parsear el schema
  if (!parser.readSchema()) {
    console.error('❌ Error leyendo schema.prisma');
    return false;
  }

  parser.parseModels();
  console.log(`✅ ${parser.models.size} modelos encontrados`);
  ...
  try {
    ...
    console.log(`\n✅ Schemas guardados en: ${outputDir}`);
    console.log(`📦 Total: ${savedCount} schemas generados`);
  } catch (error) {
